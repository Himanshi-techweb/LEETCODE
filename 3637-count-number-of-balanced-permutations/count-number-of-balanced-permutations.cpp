...
// int countWays(int pos, int evenSum, int oddSum, vector<int>& freq, int len) {
//     if (pos == len) {
//         return (evenSum == oddSum) ? 1 : 0;
//     }

//     // Create a key for memoization
//     string key = to_string(pos) + "," + to_string(evenSum) + "," + to_string(oddSum);
//     for (int i = 0; i < 10; ++i) {
...
//             freq[d]--;
//             if (pos % 2 == 0)
//                 total = (total + countWays(pos + 1, evenSum + d, oddSum, freq, len)) % MOD;
//             else
//                 total = (total + countWays(pos + 1, evenSum, oddSum + d, freq, len)) % MOD;
//             freq[d]++;
//         }
//     }
...
//     return countWays(0, 0, 0, freq, num.length());
// }
// };
using ll = long long;
const int MX = 80;
